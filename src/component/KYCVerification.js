...
export const KYCVerification = (props) => {
  ...
  const handleNext = async () => {
    ...
    if (step === 7) {
      if (wallet_address === null) {
        ...
        if (
          checkboxState.terms &&
          checkboxState.personal &&
          checkboxState.registering &&
          checkboxState.participate
        ) {
  ...
  const cancleKYC = () => {
    ...
    setPassportUrl(null);
    setUserPhotoUrl(null);
    checkboxState.terms = false;
    checkboxState.participate = false;
    checkboxState.registering = false;
    checkboxState.personal = false;
    props.onHide();
    setStep(1);
  };
                        ...
                        <ul>
                          {passport_url && (
                            <li key={passport_url.path}>
                              {passport_url.path} - {(passport_url.size / 1024).toFixed(2)} KB
                            </li>
                          )}
                        </ul>
                      )}
                        ...
                        <ul>
                          {user_photo_url && (
                            <li key={user_photo_url.path}>
                              {user_photo_url.path} - {(user_photo_url.size/1024).toFixed(2)}{" "}
                              KB
                            </li>
                          )}
                        </ul>
